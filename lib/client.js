...
function collectSyncedStorage() {
	...
	return values;
		...
		/** Parse a serialized combo ("Mod+Shift+H"). Mod (matched as ⌘ OR Ctrl on
		...
		/** Platform-aware label for tooltips and the settings row (⌘⇧H / Ctrl+Shift+H). */
		...
		/** Recording-time validation. Returns a locale error key (resolved under
		 * "shortcuts.*" by the caller) or null when the combo is safe to bind. */
		function validateRecorded(parsed, map, actionId) {
			// Bare ⌘/Ctrl+letter is browser-reserved territory (⌘W closes the
			// tab, ⌘D bookmarks…) — the page never sees those events.
			if (parsed.key.length === 1 && !parsed.shift && !parsed.alt) return "err.letter";
			...
			if (parsed.shift && !parsed.alt && (parsed.key === "w" || parsed.key === "q" || parsed.key === "t" || parsed.key === "n")) return "err.danger";
			for (const other of ACTION_IDS) {
				if (other === actionId) continue;
				if (combosEqual(parseCombo(map[other]), parsed)) return "err.dup";
			}
			return null;
		}
		...
		function normalizePrefs(raw) {
			...
			for (const cmd of ACTION_IDS) {
				...
				if (!parsed || claimed.some((c) => combosEqual(c, parsed))) parsed = parseCombo(DEFAULT_SHORTCUTS[cmd]);
		...
		function patchPrefs(current, patch) {
			...
			if (patch.shortcuts && typeof patch.shortcuts === "object") {
				for (const cmd of ACTION_IDS) {
					...
					const parsed = parseCombo(text);
					if (!parsed) continue;
					const collides = ACTION_IDS.some((other) => other !== cmd && combosEqual(parseCombo(next.shortcuts[other]), parsed));
		...
		const zh = {
			...
			"shortcuts.hint": "组合键需包含 ⌘/Ctrl；单个字母或数字需搭配 Shift 或 Alt，否则会被浏览器保留。个别组合可能仍被浏览器或系统全局热键占用，遇到无效换一个即可。",
			"shortcuts.err.noMod": "组合键需包含 ⌘ 或 Ctrl",
			"shortcuts.err.letter": "字母/数字键需搭配 Shift 或 Alt，否则会被浏览器保留",
			"shortcuts.err.danger": "该组合被浏览器或系统保留（例如会关掉所有窗口），请换一个",
			"shortcuts.err.dup": "该组合已被其他动作使用",
			"color.title": "选中高亮色",
			"color.theme": "跟随主题",
			"color.pick": "自定义…",
		...
		const en = {
			...
			"shortcuts.hint": "Combos need ⌘/Ctrl; a bare letter or digit also needs Shift or Alt, otherwise the browser keeps it. A few combos may still be taken by the browser or global hotkeys — pick another if one does nothing.",
			"shortcuts.err.noMod": "The combo needs ⌘ or Ctrl",
			"shortcuts.err.letter": "Letters/digits need Shift or Alt, otherwise the browser keeps them",
			"shortcuts.err.danger": "That combo is reserved by the browser or the OS (it would e.g. close all windows) — pick another",
			"shortcuts.err.dup": "That combo is already bound to another action",
		 ...
		 * new combination (Esc cancels); reserved/duplicate combos are rejected
		 * with an inline error instead of binding. */
		function ShortcutsRow({ t, useStore, setShortcut }) {
			...
			React.useEffect(() => {
				...
				const onKey = (event) => {
					...
					const errKey = validateRecorded(parsed, prefsRef.current.shortcuts, recording);
					if (errKey !== null) {
						setError(t("shortcuts." + errKey));
						return;
					}
			...
			return React.createElement("div", { style: settingsStyles.group },
				...
				React.createElement("div", { style: settingsStyles.actionRow },
					error !== null ? React.createElement("span", { style: settingsStyles.error }, error) : null,
					recording !== null ? React.createElement("span", { style: settingsStyles.hint }, t("shortcuts.recordHint")) : null
				),
				React.createElement("div", { style: settingsStyles.hint }, t("shortcuts.hint"))
			);
		}
