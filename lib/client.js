...
function collectSyncedStorage() {
	...
	return values;
		...
		const zh = {
			...
			"shortcuts.hint": "什么组合都能绑，主动权在你。被浏览器保留的组合（如 ⌘D 书签）或被系统全局热键占用的组合，页面可能捕捉不到；和别的动作撞车时，列表里靠前的动作先响应。绑了没反应就换一个。",
		...
		const en = {
			...
			"shortcuts.hint": "Any combo can be bound — your call. Combos kept by the browser (⌘D bookmarks…) or taken by OS global hotkeys may never reach the page, and when two actions share a combo the one listed earlier wins. Switch if one does nothing.",
