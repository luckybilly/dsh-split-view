...
function collectSyncedStorage() {
	...
	return values;
		...
		// Keep in sync with the "version" field in package.json — the client bundle
		// has no runtime version source (boot manifest entries carry no version).
		...
		const zh = {
			...
			"version.upgrade": "升级",
			"version.copied": "已复制",
			"version.upgradeHint": "在终端执行上面的命令后，重启 dsh web 生效。",
		...
		const en = {
			...
			"version.upgradeHint": "Run the command above in a terminal, then restart dsh web to apply.",
		...
		const settingsStyles = {
			...
			versionRow: { display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" },
			versionText: { color: "var(--dsw-alias-label-tertiary)", fontSize: "12px", lineHeight: "18px" },
			versionBadge: {
				display: "inline-flex",
				alignItems: "center",
				height: "18px",
			...
			// Green intentionally uses a raw hex: no theme token is known for success.
			versionBadgeOk: { color: "#1a7f37", background: "rgba(26,127,55,.12)" },
			versionBadgeBehind: { color: "var(--dsw-alias-state-warning-primary,#b8860b)", background: "rgba(184,134,11,.14)" },
			versionButton: {
				height: "24px",
				padding: "0 10px",
				borderRadius: "7px",
		...
		/**
		 * Version footer at the bottom of the settings section: current version +
		 * latest-version badge (queried straight from npmmirror/npmjs in the browser,
		 * both registries are CORS-open) + an upgrade entry when behind. Upgrade =
		 * copy the install command for the terminal; the client bundle cannot run
		 * shell itself, and a one-click upgrade would need server-side orchestration
		 * (the market plugin's approach), which this plugin deliberately avoids.
		 */
		function VersionRow({ t }) {
			...
			React.useEffect(() => {
				let alive = true;
				(async () => {
					// Debug hook: localStorage dsh-split:debugLatest forces the
					// "latest" value (exercises the upgrade path without a release).
					try {
						const forced = window.localStorage.getItem("dsh-split:debugLatest");
						if (forced && forced.trim()) {
							if (alive) setLatest(forced.trim());
							return;
						}
					} catch {
					...
					const sources = [
						"https://registry.npmmirror.com/dsh-split-view/latest",
						"https://registry.npmjs.org/dsh-split-view/latest"
					];
					for (const url of sources) {
						try {
							...
							if (data && typeof data.version === "string") {
								if (alive) setLatest(data.version.trim());
								return;
							}
						} catch {
							// try the next source
					...
					// Every source failed: stay on the current version alone —
					// the badge and the upgrade button simply do not render.
				})();
				return () => { alive = false; };
			}, []);
			const behind = latest !== null && compareVersions(PLUGIN_VERSION, latest) < 0;
			const command = "dsh plugin --profile web add dsh-split-view@latest";
			const upgrade = async () => {
				setShowCommand(true);
				try {
					...
					// clipboard refused — the command box is already on the page
				}
			};
			return React.createElement("div", { style: { padding: "14px 0 2px" } },
				React.createElement("div", { style: settingsStyles.versionRow },
					React.createElement("span", { style: settingsStyles.versionText }, t("version.current") + " v" + PLUGIN_VERSION),
					latest !== null
						? behind
							? React.createElement("span", { style: Object.assign({}, settingsStyles.versionBadge, settingsStyles.versionBadgeBehind) }, t("version.latest") + " v" + latest)
							: React.createElement("span", { style: Object.assign({}, settingsStyles.versionBadge, settingsStyles.versionBadgeOk) }, t("version.upToDate"))
						: null,
					behind ? React.createElement("button", { type: "button", style: settingsStyles.versionButton, onClick: upgrade }, copied ? t("version.copied") : t("version.upgrade")) : null
				),
				behind && showCommand ? React.createElement("div", { style: settingsStyles.versionCommand }, command) : null,
				behind && showCommand ? React.createElement("div", { style: settingsStyles.hint }, t("version.upgradeHint")) : null
			);
		}
